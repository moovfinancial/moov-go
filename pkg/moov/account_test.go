...
func TestAccountMarshalResponse_V2507(t *testing.T) {
	input := []byte(`{"mode":"sandbox","accountID":"638481a5-5205-406c-84c7-2fc2239105d1","accountType":"guest","displayName":"Guest 638481a5","profile":{"guest":{"name":"Guest 638481a5","phone":{"number":"5555555555","countryCode":"1"},"email":"test@moov.io"}},"verification":{"verificationStatus":"unverified","status":"unverified"},"createdOn":"2023-11-08T23:06:16.168497001Z","updatedOn":"2023-11-08T23:06:16.168497001Z"}`)

	account := new(mv2507.Account)
	...
	assert.Equal(t, "Guest 638481a5", account.Profile.Guest.Name)
	t.Logf("%#v", account)
}
...
func TestCreateAccountIndividual_V2507(t *testing.T) {
	account := createTestIndividualAccount_V2507()

	mc := NewTestClient(t)

	completed, started, err := mv2507.Accounts.Create(context.Background(), *mc, account)
	NoResponseError(t, err)
	require.NotNil(t, completed)
	require.Nil(t, started)
}
...
func TestGetAccount_V2507(t *testing.T) {
	mc := NewTestClient(t)

	account, err := mv2507.Accounts.Get(context.Background(), *mc, "638481a5-5205-406c-84c7-2fc2239105d1")
	NoResponseError(t, err)
	require.Equal(t, "638481a5-5205-406c-84c7-2fc2239105d1", account.AccountID)
}
...
func TestPatchIndividualAccount_V2507(t *testing.T) {
	mc := NewTestClient(t)
	ctx := BgCtx()

	account, _, err := mv2507.Accounts.Create(ctx, *mc, createTestIndividualAccount_V2507())
	NoResponseError(t, err)
	require.NotNil(t, account)
	...
	result, err := mv2507.Accounts.Patch(context.Background(), *mc, account.AccountID, patchAccount)
	NoResponseError(t, err)
	require.NotNil(t, result)
	require.NotNil(t, result.Profile.Individual)
	require.Nil(t, result.Profile.Business)
...
func TestListAccounts_V2507(t *testing.T) {
	mc := NewTestClient(t)
...
func TestDisconnectAccount_V2507(t *testing.T) {
	mc := NewTestClient(t)
	ctx := BgCtx()

	accnt, _, err := mv2507.Accounts.Create(ctx, *mc, createTestIndividualAccount_V2507())
	NoResponseError(t, err)
	require.NotNil(t, accnt)

	_, err = mv2507.Accounts.Get(ctx, *mc, accnt.AccountID)
	NoResponseError(t, err)

	err = mv2507.Accounts.Disconnect(ctx, *mc, accnt.AccountID)
	NoResponseError(t, err)
}
